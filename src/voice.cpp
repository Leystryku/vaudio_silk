...
bool VoiceCodec::Init(int quality)
{
	silk = new Voice_Silk;

	bool ret = silk->Init(quality);
	return ret;
}

void VoiceCodec::Release()
{
	silk->Release();
}

int VoiceCodec::Compress(const char *pUncompressed, int nSamples, char *pCompressed, int maxCompressedBytes, bool bFinal)
{
	return silk->Compress(pUncompressed, nSamples, pCompressed, maxCompressedBytes, bFinal);
}

int VoiceCodec::Decompress(const char *pCompressed, int compressedBytes, char *pUncompressed, int maxUncompressedBytes)
{
	return silk->Decompress(pCompressed, compressedBytes, pUncompressed, maxUncompressedBytes);
}

bool VoiceCodec::ResetState()
{
	return silk->ResetState();
}
